...
@section('title', 'Settings : Role -> Update')
        ...
        <div class="row match-height">
            <div class="col-md-12 col-12">
                <div class="card">
                    ...
                    <form method="POST" action="{{ route('admin.setting.role.update', [$role->name]) }}">
                        @method('put')
                        @csrf

                        <div class="card-header">
                            <h4 class="card-title">Update Role Record</h4>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control @error('name') is-invalid @enderror" type="text" placeholder="e.g Operator" autocomplete="off" name="name" value="{{ old('name', $role->name) }}" autofocus="" required="" tabindex="1" />
                                        @error('name')
                                        ...
                                        @foreach($submodules as $submodule)
                                            @if($submodule->group == $module->group)
                                                <div class="custom-control custom-checkbox role-name field-{{ str_replace(':','-', $submodule->name) }} " >
                                                    <input type="checkbox" class="custom-control-input {{ $module->group }}" name="submodule[]" value="{{$submodule->name}}" id="{{$submodule->name}}" {{ is_array($module_access) && in_array($submodule->name, $module_access) ? 'checked' : ''}} />
    ...
    $(function(){
        ...
        $('.filter').on('keyup keyin', function(){
            ...
            $.ajax({
                ...
                data: {'filter': filter, 'name': '{!! $role->name !!}', '_token':' {!! csrf_token() !!}' }, 
                type: "GET",
