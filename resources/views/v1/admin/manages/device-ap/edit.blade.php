...
@section('title', 'Manage : Device [ Access Point ] -> Update')
        ...
        <div class="row match-height">
            <div class="col-md-12 col-12">
                <div class="card">
                    ...
                    <form method="POST" action="{{ route('admin.manage.device-ap.update', [$device->device_uid]) }}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 pb-1">
                                    ...
                                    <div class="form-check form-switch form-check-success">
                                        <input type="checkbox" class="form-check-input" id="customSwitch11" {{ old('is_active', $device->is_active) ? 'checked' : '' }} name="is_active" value="1" />
                                ...
                                <div class="col-md-6 pb-1">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control @error('name') is-invalid @enderror" type="text" placeholder="e.g AP Sushi King Cashier AP" autocomplete="off" name="name" value="{{ old('name', $device->name) }}" autofocus="" required="" tabindex="1" />
                                        @error('name')
                            ...
                            <div class="row">
                                <div class="col-md-6 pb-1">
                                    <div class="form-group">
                                        ...
                                        <select class="select2 form-control type" name="type" tabindex="3">
                                            <option value="">Please Select</option>
                                            @foreach(config('custom.access_point_type') as $key => $val)
                                                <option value="{{ $key }}" {{ $key == old('type', $device->device_type) ? 'selected' : '' }}>{{$val}}</option>
                                            @endforeach
                                        </select>
                                        @error('type')
                                            <span class="invalid-feedback" role="alert">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="select2 form-control" name="location" id="input_location"  required="" tabindex="4">
                                            @foreach($locations as $location)
                                                <option value="{{ $location->location_uid }}" {{ old('location', $device->location_uid) ==  $location->location_uid ? 'selected' : ''  }}>{{ ucfirst($location->name) }}</option>
                                            @endforeach
                                        </select>
                                        <div>
                                ...
                                <div class="col-md-12 pb-1">
                                    ...
                                    <div class="form-check form-switch form-check-primary">
                                        <input type="checkbox" class="form-check-input" id="customSwitch10" {{ old('enable', $device->is_default_setting) == 1 ? "checked" : "" }} name="enable" value="1" tabindex="6" />
                                ...
                                <div class="col-md-6 rssi_field">
                                    <div class="form-group">
                                        <label for="name">RSSI [ WIFI ]  <span class="text-danger">*</span></label>
                                        <div id="rssi_strength" class="my-1 mb-3 mt-2"></div>
                                        <input type="hidden" name="rssi_min" value="{{ old('rssi_min', $device->rssi_min) }}" >
                                        <input type="hidden" name="rssi_enter" value="{{ old('rssi_enter', $device->rssi_max) }}">
                                        @error('rssi_min')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                ...
                                <div class="col-md-6 rssi_field">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" name="rssi_min_ble" value="{{ old('rssi_min_ble', $device->rssi_min_ble) }}" >
                                        <input type="hidden" name="rssi_enter_ble" value="{{ old('rssi_enter_ble', $device->rssi_max_ble) }}">
                                        @error('rssi_min_ble')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
...
<script type="text/javascript">
    ...
    var old_venue = {!! json_encode(old('venue', $device->venue_uid)) !!}
    var old_zone  = {!! json_encode(old('zone', $device->zone_uid)) !!}
    ...
    $(document).ready(function() {
        ...
        var old_rssi_passby = {!! json_encode(old('rssi_min', $device->rssi_min)) !!}
        var old_rssi_enter = {!! json_encode(old('rssi_enter', $device->rssi_max)) !!}


        var old_rssi_passby_ble = {!! json_encode(old('rssi_min', $device->rssi_min_ble)) !!}
        var old_rssi_enter_ble = {!! json_encode(old('rssi_enter', $device->rssi_max_ble)) !!}
